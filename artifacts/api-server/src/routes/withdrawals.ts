...
router.post("/withdrawals", requireAuth, async (req: Request, res: Response): Promise<void> => {
  ...
  if (!amount || amount <= 0 || !wallet_address) { res.status(400).json({ error: "Amount and wallet address required" }); return; }
  ...
  await db.insert(notificationsTable).values({
    ...
    message: `Withdrawal of $${amount.toFixed(2)} is pending admin approval`,
    is_read: false,
  });
